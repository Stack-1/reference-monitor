...
ssize_t onefile_write(struct kiocb *iocb, struct iov_iter *from)
{
    ...
    {
        pr_err("%s: [ERROR] Failed to copy %ld bytes from iov_iter\n", MOD_NAME, payload);
        mutex_unlock(&mutex);
        return 0;
    }

    pr_info("%s: [INFO] Trying to write string: %s", MOD_NAME, data);
